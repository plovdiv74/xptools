...
static const glyph_info_t k_glyph_metadata[] = {
    ...
    glyph_A,        "A",        "A",  1, 1, 1, 0, 0, glyph_B,         "B",      "B",  1, 1, 1, 0, 0,
    glyph_C,        "C",        "C",  1, 1, 1, 0, 0, glyph_D,         "D",      "D",  1, 1, 1, 0, 0,
    glyph_E,        "E",        "E",  1, 1, 1, 0, 0, glyph_F,         "F",      "F",  1, 1, 1, 0, 0,
    glyph_G,        "G",        "G",  1, 1, 1, 0, 0, glyph_H,         "H",      "H",  1, 1, 1, 0, 0,
    glyph_I,        "I",        "I",  1, 1, 1, 0, 0, glyph_J,         "J",      "J",  1, 1, 1, 0, 0,
    glyph_K,        "K",        "K",  1, 1, 1, 0, 0, glyph_L,         "L",      "L",  1, 1, 1, 0, 0,
    glyph_M,        "M",        "M",  1, 1, 1, 0, 0, glyph_N,         "N",      "N",  1, 1, 1, 0, 0,
    glyph_O,        "O",        "O",  1, 1, 1, 0, 0, glyph_P,         "P",      "P",  1, 1, 1, 0, 0,
    glyph_Q,        "Q",        "Q",  1, 1, 1, 0, 0, glyph_R,         "R",      "R",  1, 1, 1, 0, 0,
    glyph_S,        "S",        "S",  1, 1, 1, 0, 0, glyph_T,         "T",      "T",  1, 1, 1, 0, 0,
    glyph_U,        "U",        "U",  1, 1, 1, 0, 0, glyph_V,         "V",      "V",  1, 1, 1, 0, 0,
    glyph_W,        "W",        "W",  1, 1, 1, 0, 0, glyph_X,         "X",      "X",  1, 1, 1, 0, 0,
    glyph_Y,        "Y",        "Y",  1, 1, 1, 0, 0, glyph_Z,         "Z",      "Z",  1, 1, 1, 0, 0,

    glyph_0,        "0",        "0",  1, 1, 1, 1, 0, glyph_1,         "1",      "1",  1, 1, 1, 1, 0,
    glyph_2,        "2",        "2",  1, 1, 1, 1, 0, glyph_3,         "3",      "3",  1, 1, 1, 1, 0,
    glyph_4,        "4",        "4",  1, 1, 1, 1, 0, glyph_5,         "5",      "5",  1, 1, 1, 1, 0,
    glyph_6,        "6",        "6",  1, 1, 1, 1, 0, glyph_7,         "7",      "7",  1, 1, 1, 1, 0,
    glyph_8,        "8",        "8",  1, 1, 1, 1, 0, glyph_9,         "9",      "9",  1, 1, 1, 1, 0,

    glyph_dash,     "-",        "-",  1, 1, 0, 0, 0, glyph_dot,       "*",      "*",  1, 1, 0, 0, 0,
    glyph_period,   ".",        ".",  1, 1, 0, 0, 0, glyph_slash,     "/",      "/",  1, 1, 0, 0, 0,
    glyph_space,    "_",        "_",  1, 1, 0, 0, 0, glyph_separator, "|",      "|",  1, 1, 1, 0, 0,
    glyph_comma,    "comma",    ",",  1, 1, 0, 0, 0,

    glyph_up,       "^u",       NULL, 1, 1, 0, 0, 0, glyph_down,      "^d",     NULL, 1, 1, 0, 0, 0,
    glyph_left,     "^l",       NULL, 1, 1, 0, 0, 0, glyph_right,     "^r",     NULL, 1, 1, 0, 0, 0,
    glyph_leftup,   "^lu",      NULL, 1, 1, 0, 0, 0, glyph_rightup,   "^ru",    NULL, 1, 1, 0, 0, 0,
    glyph_leftdown, "^ld",      NULL, 1, 1, 0, 0, 0, glyph_rightdown, "^rd",    NULL, 1, 1, 0, 0, 0,

    glyph_critical, "critical", NULL, 0, 0, 0, 0, 1, glyph_hazard,    "hazard", NULL, 0, 0, 0, 0, 1,
    glyph_no_entry, "no-entry", NULL, 0, 0, 0, 0, 1, glyph_safety,    "safety", NULL, 0, 0, 0, 0, 1,
    glyph_r1,       "r1",       NULL, 1, 1, 0, 0, 0, glyph_r2,        "r2",     NULL, 1, 1, 0, 0, 0,
    glyph_r3,       "r3",       NULL, 1, 1, 0, 0, 0};

static const char* k_err_msgs[parser_error_count] = {"Invalid @ instruction",
                                                     "Not a real glyph",
...
private:
    ...
    // If the parser is in state I_ACCUM_GLYPHS, all output is dumped into the glyph buffer until we're ready to flush
    // it. In all other states, the glyph buffer is empty. Therefore at any given time when we need to accumulate an
    // error, either:
    //
    // (a) we are working on the glyph buffer, which is non-emtpy, and the current position is at the NEXT character
    // after the glyph buf's characters.  We thus know where in the source std::string the glyph buffer came from and we
    // can attribute the error to the glyph buffer or
    //
    // (b) we are parsing a single character at mPosition.
    //
    // Code that does both (e.g. if we hit a { inside a {} expression with the glyph buf non-emtpy in I_ACCUM_GLYPHS)
    // makes sure to (1) first flush the glyph buf, attributing any errors to that buffer, e.g. if the glyph name is
    // silly or the color is wrong and (2) then clears the glyph buf and copes with additional errors (e.g. nested
    // braces), attributing that to the separatolr.
...
WED_Sign_Parser::WED_Sign_Parser(const parser_in_info& input, parser_out_info& output)
    : mCurColor(sign_color_invalid), mOnFront(true), mPosition(0), mInput(input), mOutput(output)
{
}
...
void WED_Sign_Parser::append_error(parser_error_t code)
{
    ...
    stringstream str;
    str << "Chars " << e.position << " to " << e.position + e.length - 1 << ": " << k_err_msgs[code] << " "
        << mInput.input.substr(e.position, e.length);
    e.msg = str.str();
