...
static RunwayInfoVec_t CollectPotentiallyActiveRunways(const TaxiRouteInfoVec_t& all_taxiroutes,
                                                       const RunwayInfoVec_t& all_runways_info, const FlowVec_t& flows,
                                                       ATCRunwayUseVec_t& use_rules, validation_error_vector& msgs,
                                                       WED_Airport* apt)
{
    ...
    else
    {
        ...
        {
            // Search through all runway uses, testing if the runway has a match AND at least one taxiroutes associated
            // with it
            for (ATCRunwayUseVec_t::const_iterator use_itr = use_rules.begin(); use_itr != use_rules.end(); ++use_itr)
            {
                ...
                if (runway_rule.runway == runway_name_p1 || runway_rule.runway == runway_name_p2)
                {
                    ...
                    else
                    {
                        ...
                        for (const auto& taxiroute : all_taxiroutes)
                        {
                            ...
                            if (runway_itr.name == taxiroute_name ||
                                (taxiroute_name[0] = '0' && runway_itr.name == taxiroute_name.substr(1)))
                            {
                                runway_info_vec.push_back(runway_itr);
...
static TaxiRouteInfoVec_t FilterMatchingRunways(const RunwayInfo& runway_info, const TaxiRouteInfoVec_t& all_taxiroutes)
{
...
static void AssignRunwayUse(RunwayInfo& runway_info, const ATCRunwayUseVec_t& all_use_rules)
{
...
//--Centerline Checks----------------------------------------------------------
static bool AllTaxiRouteNodesInRunway(const RunwayInfo& runway_info, const TaxiRouteInfoVec_t& matching_taxiroutes,
...
static bool TaxiRouteCenterlineCheck(const RunwayInfo& runway_info, const TaxiRouteInfoVec_t& matching_taxiroutes,
                                     validation_error_vector& msgs, WED_Airport* apt)
{
    int original_num_errors = msgs.size();
    for (auto& tr : matching_taxiroutes)
    {
        ...
        if (p1_to_center_dist > METERS_TO_CENTER_THRESHOLD || p2_to_center_dist > METERS_TO_CENTER_THRESHOLD)
        {
...
static bool RunwaysTaxiRouteValencesCheck(
    const RunwayInfo& runway_info,
    const TaxiRouteNodeVec_t&
        all_matching_nodes,              // All nodes from taxiroutes matching the runway, these will come in sorted
    ...
    validation_error_vector& msgs, WED_Airport* apt)
{
    ...
    for (TaxiRouteNodeVec_t::const_iterator node_itr = all_matching_nodes.begin(); node_itr != all_matching_nodes.end();
         ++node_itr)
    {
        ...
        if (node_valence == 1)
        {
            ...
            {
                msgs.push_back(validation_error_t("Runway " + runway_info.name + "'s taxi route is not continuous",
                                                  err_atcrwy_connectivity_not_continous, *node_itr, apt));
            }
        }
        else if (node_valence >= 3)
        {
            ...
            msgs.push_back(validation_error_t("Runway " + runway_info.name + "'s taxi route is split " +
                                                  std::to_string(node_valence) + " ways at taxi route node " +
                                                  node_name,
                                              err_atcrwy_connectivity_n_split, *node_itr, apt));
        }
    }

    if (num_valence_of_1 == 0 && all_matching_nodes.size() > 0)
    {
        msgs.push_back(validation_error_t("Runway " + runway_info.name + "'s taxi route forms a loop",
                                          err_atcrwy_connectivity_forms_loop, runway_info.runway_ptr, apt));
    }
    return msgs.size() - original_num_errors == 0 ? true : false;
}
...
static WED_GISPoint* get_next_node(const WED_GISPoint* current_node, const TaxiRouteInfo& next_taxiroute)
{
...
static WED_TaxiRoute* get_next_taxiroute(const WED_GISPoint* current_node, const TaxiRouteInfo& current_taxiroute)
{
    TaxiRouteVec_t viewers = filter_viewers_by_is_runway(
        current_node, current_taxiroute.name); // The taxiroute name should equal the runway name
...
static bool TaxiRouteSquishedZCheck(
    const RunwayInfo& runway_info,
    const TaxiRouteInfo& start_taxiroute, // One of the ends of this chain of taxi routes
    CoordTranslator2 translator, validation_error_vector& msgs, WED_Airport* apt)
{
    // We know all the nodes are within threshold of the center and within bounds, the segments are parallel enough,
    // the route is a complete chain with no 3+-way splits. Now: do any of the segments make TWO complete 180
    // unexpectedly? Granted, this is an EXTREMELY rare case, because unless there are taxiway exits forking off each of
    // those sharp turns this would be caught by the generic "sudden sharp turn" test that is run on ALL taxiroutes
    ...
    while (current_node != NULL)
    {
        ...
        std::pair<bool, bool> relationship =
            get_taxiroute_relationship(current_node, current_taxiroute, next_taxiroute);
        ...
        if (real_sign != expected_sign)
        {
            ...
            msgs.push_back(validation_error_t("Taxi routes " + current_taxiroute.name + " and " + next_taxiroute.name +
                                                  " are making a turn that is too tight for aircraft to follow.",
                                              err_atcrwy_centerline_too_sharp_turn, problem_children, apt));
...
static bool FullyConnectedNetworkCheck(
    const TaxiRouteVec_t& all_taxiroutes, // All the taxiroutes in the airport, for EnsureRunwayTaxirouteValences
    validation_error_vector& msgs, WED_Airport* apt)
{
    int original_num_errors = msgs.size();
...
static bool DoTaxiRouteConnectivityChecks(
    const RunwayInfo& runway_info,
...
static bool TaxiRouteParallelCheck(const RunwayInfo& runway_info, const TaxiRouteInfoVec_t& matching_routes,
                                   validation_error_vector& msgs, WED_Airport* apt)
{
    int original_num_errors = msgs.size();
    ...
    for (auto& tr : matching_routes)
    {
        ...
        {
            std::string msg = "Taxi route segment " + tr.name + " is not parallel to the runway's " + runway_info.name +
                              "'s center line.";
...
static bool RunwayHasCorrectCoverage(const RunwayInfo& runway_info, const TaxiRouteInfoVec_t& all_taxiroutes,
                                     validation_error_vector& msgs, WED_Airport* apt)
{
    ...
    if (length_accumulator < COVERAGE_THRESHOLD)
    {
        ...
        msgs.push_back(
            validation_error_t(msg, err_atcrwy_taxi_route_does_not_span_enough_rwy, runway_info.runway_ptr, apt));
        return false;
    }
                         ...
                         WED_Airport* apt)                     // Airport to pass into msgs
{
    ...
    for (std::set<std::string>::const_iterator hot_set_itr = hot_set.begin(); hot_set_itr != hot_set.end();
         ++hot_set_itr)
    {
    ...
    {
        stringstream ss;
        ss << "Taxi route " << taxiroute.name << " is too close to runway " << ENUM_Desc(runway_number)
           << " and now must be marked active for runway " << ENUM_Desc(runway_number) << " " << op_type;

        msgs.push_back(validation_error_t(ss.str(), err_atcrwy_hotzone_taxi_route_too_close, taxiroute.ptr, apt));
    }
    return !found_marked;
}

// Returns polygon in lat/lon
static Polygon2 MakeHotZoneHitBox(
    const RunwayInfo& runway_info, // The relevant runway info
    ...
    bool make_arrival)             // add arrival protection area, but *only if* runway is used for arrivals
{
    ...
    // Unfortunatly due to the messy real world we must have unrealistically low thresholds to avoid edge case after
    // edge case for very airport that doesn't play by the rules, was grandfathered in, or was built on Mt. Doom and
    // needs to avoid the volcanic dust clouds. You know, the usual.
    ...
    if (!paved || runway_info.runway_ptr->GetLength() < 1500.0 || runway_info.runway_ptr->GetWidth() < 20.0)
        HITZONE_WIDTH_THRESHOLD_M =
            10.0; // good guess, FAA advisory 150' from CL is impractically wide for many small airfields
...
static bool DoHotZoneChecks(const RunwayInfo& runway_info, const TaxiRouteInfoVec_t& all_taxiroutes,
                            const std::vector<WED_RampPosition*>& ramps, validation_error_vector& msgs,
                            WED_Airport* apt)
{
    int original_num_errors = msgs.size();
    ...
    for (int runway_side = 0; runway_side < 2; ++runway_side)
    {
        ...
        {
            // Make the hitbox based on the runway and which side (low/high) you're currently on and if you need to be
            // making arrival or departure
            ...
            for (auto& taxiroute_itr : all_taxiroutes)
            {
                ...
                if (hit_box.intersects(taxiroute_itr.segment_geo) || hit_box.inside(taxiroute_itr.segment_geo.p1))
                {
                    ...
                    {
                        hitbox_error |= FindIfMarked(runway_number, taxiroute_itr, taxiroute_itr.hot_arrivals,
                                                     "arrivals", msgs, apt);
                    }

                    if (runway_info.IsHotForDeparture(runway_number) && static_cast<bool>(make_arrival) == false)
                    {
                        hitbox_error |= FindIfMarked(runway_number, taxiroute_itr, taxiroute_itr.hot_departures,
                                                     "departures", msgs, apt);
                    }
                }
            }
    ...
    if (ramps_near_rwy.size())
        msgs.push_back(validation_error_t("Only Ramp Starts of type=misc are allowed near runways",
                                          err_ramp_only_misc_starts_in_hotzones, ramps_near_rwy, apt));
    return msgs.size() - original_num_errors == 0 ? true : false;
}

// flag all ground traffic routes that cross a runways hitbox

static void AnyTruckRouteNearRunway(const RunwayInfo& runway_info, const TaxiRouteInfoVec_t& all_routes,
                                    const std::vector<WED_RoadEdge*>& roads, validation_error_vector& msgs,
                                    WED_Airport* apt)
{
    Polygon2 runway_hit_box(runway_info.corners_geo);

    Vector2 side_ext = runway_info.width_vec_1m *
                       (5.0 + 0.5 * runway_info.runway_ptr->GetWidth()); // to centerline of road. Edge-2-edge spacing
                                                                         // is 5-10m less, depending on road width.
    Vector2 len_ext = runway_info.dir_vec_1m *
                      (runway_info.runway_ptr->GetLength() > 1500.0 ? 60.0 : 30.0); // to go around end of runway
    ...
    if (close_routes.size() || close_roads.size())
    {
        ...
        if (close_roads.size())
            msgs.push_back(validation_error_t("Road too close to runway", err_atcrwy_truck_route_too_close_to_runway,
                                              close_roads, apt));
                               ...
                               validation_error_vector& msgs, WED_Airport* apt, WED_ResourceMgr* rmgr)
{
    ...
    for (auto pp : all_polygons)
    {
        if (pp->Cull(runway_bounds))
        {
            ...
            if (lg <= group_RunwaysEnd)
                break; // don't worry about polygons drawn underneath the runway
            ...
            if (isOnRunway)
            {
                std::string msg;
                pp->GetName(msg);
                msg = "The gateway discourages user created runway markings. DrapedPolygon '" + msg +
                      "' intersects with runway " + runway_info.name;
...
static void TJunctionCrossingTest(const TaxiRouteInfoVec_t& all_taxiroutes, validation_error_vector& msgs,
                                  WED_Airport* apt)
{
/*For each edge A
    for each OTHER edge B

    If A and B intersect, do not mark them as a T - the intersection test will pick this up and we don't want to have
   double errors on a single user problem. else If A and B share a common vertex, do not mark them as a T junction -
   this is legal. (Do this by comparing the Point2, not the IGISPoint *.If A and B have separate exactly on top of each
   other nodes, the duplicate nodes check will find this, and again we don't want to squawk twice on one user error.
...
#define STR(s) #s
    ...
    for (auto tr_a = all_taxiroutes.cbegin(); tr_a != all_taxiroutes.cend(); ++tr_a)
    {
        ...
        for (auto tr_b = tr_a + 1; tr_b != all_taxiroutes.end(); ++tr_b)
        {
            ...
            if (edge_a.p1 == edge_b.p1 || edge_a.p1 == edge_b.p2 || edge_a.p2 == edge_b.p1 || edge_a.p2 == edge_b.p2)
                continue;
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                double dist_b_node_to_a_edge =
                    i ? edge_a.squared_distance(edge_b.p2) : edge_a.squared_distance(edge_b.p1);
                double dist_min;
                if (tr_a->is_aircraft_route)
                {
                    switch (tr_a->ptr->GetWidth())
                    {
                    case width_A:
                        dist_min = 4.5 * TJUNCTION_THRESHOLD_AC_REL;
                        break;
                    case width_B:
                        dist_min = 6.0 * TJUNCTION_THRESHOLD_AC_REL;
                        break;
                    case width_C:
                        dist_min = 9.0 * TJUNCTION_THRESHOLD_AC_REL;
                        break;
                    case width_D:
                        dist_min = 14.0 * TJUNCTION_THRESHOLD_AC_REL;
                        break;
                    case width_E:
                        dist_min = 14.0 * TJUNCTION_THRESHOLD_AC_REL;
                        break;
                    case width_F:
                        dist_min = 16.0 * TJUNCTION_THRESHOLD_AC_REL;
                        break;
                    }
                }
                else
                    dist_min = TJUNCTION_THRESHOLD_TRUCKS;
                ...
                if (dist_b_node_to_a_edge < dist_min * dist_min)
                {
                    ...
                    if (node_viewers.size() == 1)
                    {
                        ...
                        msgs.push_back(
                            validation_error_t("Taxi route " + tr_a->name + " is not joined to destination route.",
                                               err_taxi_route_not_joined_to_dest_route, problem_children, apt));
                    }
                }
                double dist_a_node_to_b_edge =
                    i ? edge_b.squared_distance(edge_a.p2) : edge_b.squared_distance(edge_a.p1);
                ...
                if (dist_a_node_to_b_edge < dist_min * dist_min)
                {
                    ...
                    if (node_viewers.size() == 1)
                    {
                        ...
                        msgs.push_back(
                            validation_error_t("Taxi route " + tr_b->name + " is not joined to a destination route.",
                                               err_taxi_route_not_joined_to_dest_route, problem_children, apt));
                    }
                }
            }
    ...
    for (auto e : crossing_edges)
        msgs.push_back(
            validation_error_t("Airport contains crossing ATC routing lines with no node at the crossing point."
                               " Split the lines and join the nodes.",
                               err_airport_ATC_network, e, apt));
    for (auto e : short_edgesAB)
        msgs.push_back(validation_error_t(std::string("Airport contains short (<") +
                                              std::to_string((int)SHORT_THRESHOLD_AC_SM) + "m) Taxi route segment(s).",
                                          grievance, e, apt));
    for (auto e : short_edgesC)
        msgs.push_back(validation_error_t(std::string("Airport contains short (<") +
                                              std::to_string((int)SHORT_THRESHOLD_AC) + "m) Taxi route segment(s).",
                                          grievance, e, apt));
    for (auto e : short_edgesDEF)
        msgs.push_back(validation_error_t(std::string("Airport contains short (<") +
                                              std::to_string((int)SHORT_THRESHOLD_AC_LG) + "m) Taxi route segment(s).",
                                          grievance, e, apt));
    for (auto e : short_edgesT)
        msgs.push_back(validation_error_t(std::string("Airport contains short (<") +
                                              std::to_string((int)SHORT_THRESHOLD_TRUCKS) +
                                              "m) Truck route segment(s).",
                                          grievance, e, apt));
}

static void TestInvalidHotZOneTags(const TaxiRouteInfoVec_t& taxi_routes, const std::set<int>& legal_rwy_oneway,
                                   const std::set<int>& legal_rwy_twoway, validation_error_vector& msgs,
                                   WED_Airport* apt)
{
    for (auto& tr : taxi_routes)
    {
        ...
        //			{
        //				msg = "This taxi route has no name.  All taxi routes must have a name so that ATC can give taxi
        // instructions.";
        //			}

        if (tr.ptr->HasInvalidHotZones(legal_rwy_oneway))
        {
            msgs.push_back(validation_error_t(std::string("Taxi route '") + tr.name +
                                                  "' has hot zones for runways not present at its airport.",
        ...
        if (tr.ptr->IsRunway())
        {
            ...
            {
                msgs.push_back(validation_error_t(std::string("Taxi route '") + tr.name +
                                                      "' is std::set to a runway not present at the airport.",
            ...
            for (int i = 0; i < 2; i++)
                if (get_node_valence(tr.nodes[i]) < 2)
                {
                    msgs.push_back(validation_error_t(
                        "Taxi routes with HotZone tags most be connected on both ends to other taxi routes.",
...
static void TwyNameCheck(const TaxiRouteInfoVec_t& all_taxiroutes_info, validation_error_vector& msgs, WED_Airport* apt)
{
    ...
    for (auto& tr : all_taxiroutes_info)
    {
        if (tr.is_aircraft_route && !tr.ptr->IsRunway())
        {
            ...
            else if (!tr.name.empty())
            {
                ...
                if (tr.name.size() > 1 && !isalnum(tr.name[1]))
                    ok = false;
                if (tr.name.size() > 2 && !isdigit(tr.name[2]))
                    ok = false;
                if (!ok)
    ...
    for (auto t : long_named)
        msgs.push_back(validation_error_t(std::string("Taxi route '") + t.first +
                                              "' name is unusually long, should be less than 4 characters.",
                                          warn_taxi_route_name_unusual, t.second, apt));

    for (auto t : odd_named)
        msgs.push_back(validation_error_t(
            std::string("Taxi route '") + t.first +
                "' name is likely wrong, should be 1-2 letters optionally followed by 1-2 digits or empty.",
            warn_taxi_route_name_unusual, t.second, apt));
}
...
void WED_DoATCRunwayChecks(WED_Airport& apt, validation_error_vector& msgs, const TaxiRouteVec_t& all_taxiroutes_plain,
                           const RunwayVec_t& all_runways, const std::set<int>& legal_rwy_oneway,
                           const std::set<int>& legal_rwy_twoway, const FlowVec_t& all_flows, WED_ResourceMgr* res_mgr,
                           const std::vector<WED_RampPosition*>& ramps, const std::vector<WED_RoadEdge*>& roads)
{
    Bbox2 box;
    apt.GetBounds(gis_Geo, box);
    CoordTranslator2 translator;
    CreateTranslatorForBounds(box, translator); // equivalent to MapZoomerNew: Pre-calculates cos(lat) to covert
                                                // LL->Meter with linear algebra, only
    ...
    if (!all_aircraftroutes.empty())
    {
        ...
        RunwayInfoVec_t potentially_active_runways =
            CollectPotentiallyActiveRunways(all_aircraftroutes, all_runways_info, all_flows, all_use_rules, msgs, &apt);
            ...
            if (!matching_taxiroutes.empty())
            {
                if (AllTaxiRouteNodesInRunway(runway_info, matching_taxiroutes, msgs, &apt))
                {
                    if (TaxiRouteParallelCheck(runway_info, matching_taxiroutes, msgs, &apt))
                    {
                        ...
                        {
                            if (DoTaxiRouteConnectivityChecks(runway_info, all_aircraftroutes, matching_taxiroutes,
                                                              translator, msgs, &apt))
                            {
