...
#define KPIXELS                                                                                                        \
    2 // maximum texture size per side in kibi-pixels before splitting up orthos at import into smaller chunks
...
static Point2 interpol_LonLat(double UV_x, double UV_y, double dUV, const Point2* LonLat, int y_stride)
{
    ...
    return Point2(BilinearInterpolate2d(LonLat[A].x(), LonLat[B].x(), LonLat[C].x(), LonLat[D].x(), UV_x, 1.0 - UV_y),
                  BilinearInterpolate2d(LonLat[A].y(), LonLat[B].y(), LonLat[C].y(), LonLat[D].y(), UV_x, 1.0 - UV_y));
}
...
void WED_MakeOrthos(IResolver* inResolver, WED_MapZoomerNew* zoomer)
{
    ...
    if (path)
    {
        ...
        while (*path)
        {
            ...
            if (base_tex.find(" ") == base_tex.npos && img_path[0] != '.' && img_path[0] != DIR_CHAR &&
                img_path[1] != ':')
            {
                std::vector<Point2> gcp;
                ...
                if (rng0)
                {
                    ...
                    if (tref != NULL)
                    {
                        ...
                        if (kopt_x < kpix_x || kopt_y < kpix_y)
                        {
                            ...
                            {
                                stringstream msg;
                                auto w = (kpix_x * 1024 == orig_x && kpix_y * 1024 == orig_y)
                                             ? "This image is exactly "
                                             : "This image will be upscaled to ";
                                msg << w << " " << kpix_x << " x " << kpix_y << " Kpix, ";
                                msg.precision(1);
                                msg << std::fixed;
                                msg << LonLatDistMeters(corner[0], corner[1]) / (kpix_x * 1024.0) << " x "
                                    << LonLatDistMeters(corner[0], corner[3]) / (kpix_y * 1024.0) << " m/pix\n";
                                msg << "Total DDS texture size ~" << intround(kpix_x * kpix_y * 0.7) << " MB\n";
                                msg << "\n";
                                msg << "Use a space-optimized resolution of " << kopt_x << " x " << kopt_y << " Kpix, ";
                                msg << LonLatDistMeters(corner[0], corner[1]) / (kopt_x * 1024.0) << " x "
                                    << LonLatDistMeters(corner[0], corner[3]) / (kopt_y * 1024.0) << " m/pix\n";
                                msg << "Total DDS texture size ~" << intround(kopt_x * kopt_y * 0.7)
                                    << " MB instead ?\n";
                        ...
                        while (x0 < kpix_x)
                        {
                            ...
                            while (y0 < kpix_y)
                            {
                                ...
                                {
                                    df = 1.0 / (gcp_divs - 1);
                                    pts = gcp.data(); // use ground control points, i.e. place the subtextures to
                                                      // propper projected/warped locations
                                }
                                ...
                                dpol->SetSubTexture(
                                    b); // Turn on auto-redraping. So one can punch holes into them w/o distortion.

                                std::string::size_type pos = base_tex.find_last_of('.');
                                char s[10] = "";
                                if (x0 > 0 || y0 > 0)
                                    snprintf(s, 10, "+%dk+%dk", x0, y0);
                                dpol->SetName(base_tex.substr(0, pos) + s + base_tex.substr(pos));

                                y0 = y1;
                                y1 += largest_pow2(kpix_y - y1, KPIXELS);
                            }
                            x0 = x1;
                            x1 += largest_pow2(kpix_x - x1, KPIXELS);
                        }
                    }
            ...
            else
            {
                char msg[200];
                snprintf(msg, 200,
                         "Orthoimage name/path not acceptable:\n\n%s\n\n"
                         "Spaces are not allowed and location must be inside this sceneries directory.",
                         img_path.c_str());
                DoUserAlert(msg);
            }
            path += strlen(path) + 1;
