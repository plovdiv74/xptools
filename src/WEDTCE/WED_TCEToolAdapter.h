...
/*
    ...
    This needs more docs, but basically: the control handle base tool has a HUGE amount of smart behavior to make
   structured CAD-style editing easy.

    The basic plug-in format for the TCE is different from the map.  So our base classes are different.

    This class ATTEMPTS to recycle a map tool into a TCE tool by mapping the overrides.  For some map tools there is no
   way in hell that this can wokr, but - at our own risk, we can attempt to do so.  This is being done to recycle the
   handle tool base into the TCE, so that handle-based editing can be the same everywhere...the handel tool is just way,
   way, way too much code to have in two places at once.
...
{
public:
    WED_TCEToolAdapter(const char* tool_name, GUI_Pane* host, WED_MapZoomerNew* zoomer, IResolver* resolver,
    ...
    virtual void KillOperation(
        bool mouse_is_down); // Called when another tool is picked.  If a shape is half built, ABORT!
    ...
    virtual void GetNthPropertyDict(int n, PropertyDict_t& dict)
        const; // Ben says: dictionary ops are broken out (and one vs all lookup are split too) for performance.
    virtual void GetNthPropertyDictItem(int n, int e, std::string& item)
        const; // It may be slow to get all enums, so give the UI code a way to say if it needs this info.
