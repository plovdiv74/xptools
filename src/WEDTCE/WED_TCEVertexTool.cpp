...
WED_TCEVertexTool::WED_TCEVertexTool(const char* tool_name, GUI_Pane* host, WED_MapZoomerNew* zoomer,
                                     IResolver* resolver)
...
void WED_TCEVertexTool::GetNthControlHandle(intptr_t id, int n, bool* active, HandleType_t* con_type, Point2* p,
        ...
        if ((pt_bt = dynamic_cast<IGISPoint_Bezier*>(who)) != NULL)
            ...
            case 1:
                ...
                *active = pt_bt->GetControlHandleLo(gis_UV, *p);
                ...
                {
                    Point2 dummy;
                    pt_bt->GetLocation(gis_UV, dummy);
                    *direction = Vector2(*p, dummy);
                }
                break;
            case 2:
                *con_type = handle_Bezier;
...
void WED_TCEVertexTool::GetNthLinkInfo(intptr_t id, int n, bool* active, LinkType_t* ltype) const
{
    ...
    if (who->GetGISClass() == gis_Point_Bezier)
    {
        ...
        if (ltype && pt_bt)
        {
            Point2 p;
            if (n == 0)
                *ltype = pt_bt->GetControlHandleHi(gis_UV, p) ? link_BezierCtrl : link_None;
            else
                *ltype = pt_bt->GetControlHandleHi(gis_UV, p) ? link_BezierCtrl : link_None;
        }
    }
...
void WED_TCEVertexTool::ControlsLinksBy(intptr_t id, int c, const Vector2& delta, Point2& io_pt)
{
    ...
    case gis_Chain:
        ...
        if (s)
        {
            ...
            Point2 p;
            p1->GetLocation(gis_UV, p);
            p += delta;
            p1->SetLocation(gis_UV, p);
            p2->GetLocation(gis_UV, p);
            p += delta;
            p2->SetLocation(gis_UV, p);
        }
        break;
    }
    // now that we used TCE to modify the UVmaping, update UVbounds
...
void WED_TCEVertexTool::HandleSnap(Point2& io_pt, const Vector2& delta)
{
    if (mGrid.value != tce_Grid_None)
    {
        ...
        {
        case tce_Grid_3x3:
            sub_div_h = sub_div_v = 3;
            break;
        case tce_Grid_4x4:
            sub_div_h = sub_div_v = 4;
            break;
        case tce_Grid_5x5:
            sub_div_h = sub_div_v = 5;
            break;
        case tce_Grid_3x4:
            sub_div_h = 3;
            sub_div_v = 4;
