...
void PolygonFromCCB(Pmwx::Ccb_halfedge_const_circulator circ, Polygon_2& out_poly, RingInset_t* out_inset, Inset_f func,
                    Bbox_2* extent)
{
    out_poly.clear();
    ...
    do
    {
        ...
        if (out_inset && func)
            out_inset->push_back(func(circ));
        if (extent)
            *extent += circ->source()->point().bbox();
    } while (stop != ++circ);
}

void PolygonFromFace(Pmwx::Face_const_handle in_face, Polygon_with_holes_2& out_ps, PolyInset_t* out_inset,
                     Inset_f func, Bbox_2* extent)
{
    ...
    for (Pmwx::Hole_const_iterator h = in_face->holes_begin(); h != in_face->holes_end(); ++h)
    {
        ...
        PolygonFromCCB(*h, holes.back(), out_inset ? &out_inset->back() : NULL, func,
                       NULL); // hole can't increase extent, plus passing it here nulls it out!
    }
    out_ps = Polygon_with_holes_2(outer_ccb, holes.begin(), holes.end());
}

static void PolygonFromFaceExCCB(Pmwx::Halfedge_const_handle in_edge, Polygon_2& out_ccb, RingInset_t* out_insets,
                                 Inset_f inset_f, CoordTranslator_2* xform, Simplify_f simplify_f)
{
...
void PolygonFromFaceEx(Pmwx::Face_const_handle in_face, Polygon_with_holes_2& out_ps, PolyInset_t* out_insets,
                       Inset_f func, CoordTranslator_2* xform, Simplify_f simplify)
{
    Polygon_2 outer_ccb;
    vector<Polygon_2> holes;
    ...
    PolygonFromFaceExCCB(in_face->outer_ccb(), outer_ccb, out_insets ? &out_insets->back() : NULL, func, xform,
                         simplify);
...
{
    if (!io_poly.is_unbounded())
        MapPolygonForward(io_poly.outer_boundary(), translator);
...
{
    if (!io_poly.is_unbounded())
        MapPolygonReverse(io_poly.outer_boundary(), translator);
    for (Polygon_with_holes_2::Hole_iterator h = io_poly.holes_begin(); h != io_poly.holes_end(); ++h)
...
void cgal2ben(const Polygon_with_holes_2& cgal, vector<Polygon2>& ben)
{
    ...
    if (!cgal.is_unbounded())
        cgal2ben(cgal.outer_boundary(), ben.back());
...
static Face_handle face_for_curve(Vertex_handle v, Segment_2 s)
{
    ...
    while (!is_between_cw(cv, to_left, curr->curve(), (curr->direction() == CGAL::ARR_RIGHT_TO_LEFT), next->curve(),
                          (next->direction() == CGAL::ARR_RIGHT_TO_LEFT), v->point(), eq_curr, eq_next))
    {
        if (eq_curr || eq_next)
            return Face_handle();
            ...
            if (v1 != v2)
                if (*v1 != *v2)
                {
                    ...
                    if (len < dsqr)
                    {
                        ...
                        //			debug_mesh_line(cgal2ben(possible_curve.source()),cgal2ben(possible_curve.target()),1,0,0,
                        // 0,1,0);
                        seg_queue.insert(
                            seg_queue_t::value_type(len, std::pair<Vertex_handle, Vertex_handle>(*v1, *v2)));
                    }
                }
...
void SafeMakeMoreConvex(Polygon_set_2& ioPolygon, double max_area)
{
    ...
    do
    {
        ...
        do
        {
            ...
            if (third->target() != iter->source() && third != iter && CGAL::to_double(tri.area()) < max_area &&
                CGAL::left_turn(iter->source()->point(), iter->target()->point(), next->target()->point()))
            {
                //				bool ok = true;
...

                {
                    ...
                    pmwx.insert_at_vertices(
                        X_monotone_curve_2(Segment_2(iter->source()->point(), next->target()->point())), iter->source(),
                        next->target());
                    ++ctr;
                    did_work = true;
                    break; // MUST break here - insert line changes topology, which can cause stop and circ to not
                           // match.  BAd!
                }
            }
            ++iter;
...
void SimplifyPolygonMaxMove(Polygon_set_2& ioPolygon, double max_err)
{
    ...
    if (ioPolygon.is_empty())
        return;
    //	printf("Before: %d polys, %d vertices.\n", ioPolygon.arrangement().number_of_faces(),
    // ioPolygon.arrangement().number_of_vertices());
    Pmwx pmwx(ioPolygon.arrangement());
...
// ioPolygon.arrangement().number_of_vertices());
    ...
    for (h = pmwx.unbounded_face()->holes_begin(); h != pmwx.unbounded_face()->holes_end(); ++h)
    {
        ...
        do
        {
            ...
            //			if(CGAL::right_turn(circ->source()->point(),
            // circ->target()->point(),circ->next()->target()->point()))
            {
...
{
    if ((extent.xmax() - extent.xmin()) <= (r * 2.0))
        return true;
    if ((extent.ymax() - extent.ymin()) <= (r * 2.0))
        return true;
