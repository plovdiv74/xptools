...
template <typename Tr, typename Lcp> class Triangulation_conformer_any_2
{
...
public:
    ...
    Triangulation_conformer_any_2(Tr& tr_)
        : initialized(NONE), tr(tr_), null_level(), null_visitor(), clusters(tr_),
          edges_level_any(tr, clusters, null_level)
    {
    }

public: /** \name ACCESS TO CLUSTERS */
    typedef typename Mesh_2::Clusters<Tr>::Cluster_vertices_iterator Cluster_vertices_iterator;
    typedef typename Mesh_2::Clusters<Tr>::Vertices_in_cluster_iterator Vertices_in_cluster_iterator;
...
public:
    ...
    //@{
    void set_initialized(Initialization init)
    {
        initialized = init;
    }
    Initialization get_initialized() const
    {
        return initialized;
    }
    //@}
    ...
    {
        int nedges = 0;
        for (typename Tr::Finite_edges_iterator eit = tr.finite_edges_begin(); eit != tr.finite_edges_end(); ++eit)
    ...
    std::pair<Vertices_in_cluster_iterator, Vertices_in_cluster_iterator> vertices_in_cluster_sequence(
        const Vertex_handle& vh, const unsigned int n)
    {
...
public:
    ...
    bool is_conforming_any()
    {
        ...
        for (Finite_edges_iterator ei = tr.finite_edges_begin(); ei != tr.finite_edges_end(); ++ei)
            if (ei->first->is_constrained(ei->second) && !is_locally_conform(this, ei->first, ei->second))
                return false;
        return true;
    }
...
template <typename Tr, typename Lcp> void make_conforming_any_2(Tr& t)
{
