...
RAII_CurlHandle::RAII_CurlHandle(const std::string& url, int buf_reserve_size)
    : m_dest_buffer(std::vector<char>(buf_reserve_size)), m_curl_handle(url, &m_dest_buffer)
{
}
...
//--RAII_File------------------------------------------------------------------
RAII_FileHandle::RAII_FileHandle(const std::string& fname, const std::string& mode)
    : mFile(fopen(fname.c_str(), mode.c_str()))
{
}
