...
CSVParser::CSVParser(char delimiter, const std::string& input) : mDelimiter(delimiter), mInput(input)
{
}

CSVParser::CSVTable CSVParser::ParseCSV()
{
    CSVTable invalid_table = CSVTable(CSVParser::CSVTable::CSVHeader(), std::vector<CSVParser::CSVTable::CSVRow>());
    // Find newline
    ...
    {
        std::vector<CSVTable::CSVRow> rows;
        // Pushback our header
        ...
        return CSVTable(rows[0], std::vector<CSVParser::CSVTable::CSVRow>(rows.begin() + 1, rows.end()));
    }
}

CSVParser::FSM CSVParser::LookupTable(FSM current_state, int pos, std::string& token)
{
    char c = mInput[pos];
...
std::string csv_parser_generate_test(bool make_junk, int str_len)
{
    std::string output = make_junk ? "BAD" : "GOOD";
...
void csv_parser_run_tests(int count)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		std::string str = csv_parser_generate_test(make_junk, str_len);
