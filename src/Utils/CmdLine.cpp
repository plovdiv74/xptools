...
static CmdLine::storage_type parse_windows_style(const char* cmdline)
{
    ...
    while (*p)
    {
        while (*p && isspace(*p))
            ++p;
        const char* wb = p;
        const bool whole_arg_is_quoted =
            (*wb == '"'); // the Python "subprocess" module does this, rather than quoting the stuff after the =
...
static void test()
{
    const std::vector<test_case> test_cases = {
        ...
        {"--foo=\"val with spaces\" --bar=baz --bang",
         {"app.sh", "--foo=\"val with spaces\"", "--bar=baz", "--bang"},
         {{"--foo", "val with spaces"}, {"--bar", "baz"}, {"--bang", ""}}},
        // Preserves case
        {"--aBc=DeF --gHi=\"JkL mNo\" --pQr",
         {"app.sh", "--aBc=DeF", "--gHi=\"JkL mNo\"", "--pQr"},
         {{"--aBc", "DeF"}, {"--gHi", "JkL mNo"}, {"--pQr", ""}}},
        // Short options
        {"-a -b -c=\"has spaces\"",
         {"app.sh", "-a", "-b", "-c=\"has spaces\""},
         {{"-a", ""}, {"-b", ""}, {"-c", "has spaces"}}},
    ...
    const bool tests_passed =
        std::all_of(test_cases.begin(), test_cases.end(), [](const test_case& tc) { return tc.test_passes(); });
