...
//  Copyright (C) 2002 Tanguy FautrÈ.
...
public:
    ...
    public:
        ...
        {
            return m_OutArcs.empty();
        }

        size_t number_of_out_arcs() const
        {
            return m_OutArcs.size();
        }
    ...
    friend void swap(_mytype& Left, _mytype& Right)
    {
        Left.swap(Right);
    }
...
inline const typename graph_array<nodetype, arctype>::node& graph_array<nodetype, arctype>::operator[](
    const nodeid& i) const
{
...
inline typename graph_array<nodetype, arctype>::const_node_reverse_iterator graph_array<nodetype, arctype>::rbegin()
    const
{
...
inline typename graph_array<nodetype, arctype>::out_arc_iterator graph_array<nodetype, arctype>::insert_arc(
...
inline typename graph_array<nodetype, arctype>::out_arc_iterator graph_array<nodetype, arctype>::insert_arc(
    const nodeid& Initial, const nodeid& Terminal, const arctype& Elem)
{
...
inline typename graph_array<nodetype, arctype>::out_arc_iterator graph_array<nodetype, arctype>::insert_arc(
    const node_iterator& Initial, const node_iterator& Terminal, const arctype& Elem)
{
...
inline typename graph_array<nodetype, arctype>::out_arc_iterator graph_array<nodetype, arctype>::erase_arc(
    const out_arc_iterator& Pos)
{
