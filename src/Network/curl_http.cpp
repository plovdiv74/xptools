...
int atomic_load(volatile int* a)
{
    return *a;
}
void atomic_store(volatile int* a, int v)
{
    *a = v;
}
...
curl_http_get_file::curl_http_get_file(const std::string& inURL, const std::string& outDestFile)
    : m_progress(-1), m_status(in_progress), m_halt(0), m_dest_path(outDestFile), m_url(inURL), m_dest_buffer(NULL),
      m_errcode(0), m_last_dl_amount(0.0)
{
    UTL_http_encode_url(m_url);

    DebugAssert(inURL.size() > 7);
    DebugAssert(strncmp(inURL.c_str(), "http://", 7) == 0 || strncmp(inURL.c_str(), "https://", 8) == 0);
...
curl_http_get_file::curl_http_get_file(const std::string& inURL, std::vector<char>* outDestBuffer)
    : m_progress(-1), m_status(in_progress), m_halt(0), m_url(inURL), m_dest_buffer(outDestBuffer), m_errcode(0),
...
curl_http_get_file::curl_http_get_file(const std::string& inURL, const std::string* post_data,
                                       const std::string* put_data, std::vector<char>* outBuffer)
    : m_progress(-1), m_status(in_progress), m_halt(0), m_url(inURL), m_post(post_data ? *post_data : std::string()),
      m_put(put_data ? *put_data : std::string()), m_dest_buffer(outBuffer)
{
    UTL_http_encode_url(m_url);
...
int curl_http_get_file::progress_cb(void* ptr, double TotalToDownload, double NowDownloaded, double TotalToUpload,
                                    double NowUploaded)
{
    ...
    else // gzipped transfers dont know the total transfer size ahead of time, so we report in kB, as a *negative*
         // number
...
curl_http_get_file::thread_proc(void* param)
{
    ...
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION,
                     true); // Required because we do a redirect to protect against URL/Server changes breaking URLs
    ...
    curl_easy_setopt(curl, CURLOPT_ACCEPT_ENCODING,
                     ""); // empty std::string is expanded into all methods supported by this version of curl.
#if WED
...
#include "WED_Version.h"
    curl_easy_setopt(curl, CURLOPT_USERAGENT,
                     "WorldEditor/" WED_VERSION_STRING_SHORT); // OSM tile server requires a referer std::string
#endif
#if 1 // DEV
...
bool UTL_http_is_error_bad_net(int err)
{
    ...
    if (err == CURLE_OPERATION_TIMEDOUT)
        return true;
    if (err == CURLE_PARTIAL_FILE)
        return true; // We get these if we don't get a complete transfer - maybe someone hung up
    if (err == CURLE_GOT_NOTHING)
        return true; // due to flakey internet?
    if (err == CURLE_COULDNT_RESOLVE_PROXY)
        return true; // Maybe we can't even connect due to this
    if (err == CURLE_COULDNT_RESOLVE_HOST)
        return true; // range of problems...maybe no net route to server.
