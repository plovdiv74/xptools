...
static void lines_to_nearest(const std::vector<Segment2>& starts, const std::vector<Segment2>& edges,
                             const Bbox2& screen_bounds, double err_2nd, GUI_GraphState* g)
{
    if (starts.size() * edges.size() > 10000)
        return; // skip drawing too complex scenarios - takes too long
    ...
    for (const auto& pix : starts)
    {
        ...
        for (const auto& pix_seg : edges)
        {
            ...
            if (dist < next_dist)
            {
                if (dist < nearest_dist)
                {
                    ...
                    nearest_dist = dist;
                    next_pix = nearest_pix;
                    nearest_pix = pix_seg.p1;
                }
                else if (Vector2(nearest_pix, pix_seg.p1).squared_length() >
                         0.1 * dist)  // only take 2nd nearest if it goes to a noticeably different location
        ...
        // avoid showing lines if its not certain that all other edges that could be closer are on-screen, i.e. tested
        // for the alternative would be to always test against ALL edges at that airport - which takes a lot of time at
        // large apts e.g. KATL with ~900 taxi and truck edges and 120 starts
        ...
        if (nearest_dist < edge_dist * edge_dist)
        {
            ...
            if (sqrt(next_dist) < sqrt(nearest_dist) + err_2nd)
                if (Vector2(next_pix, nearest_pix).squared_length() >
                    0.1 * next_dist) // dont draw second if its going to almost same location.
                {
...
void WED_ATCLayer_DrawAircraft(WED_RampPosition* pos, GUI_GraphState* g, WED_MapZoomerNew* z)
{
    ...
    {
    case width_A:
        id = GUI_GetTextureResource("ClassA.png", tex_Linear | tex_Mipmap, NULL);
        break;
    case width_B:
        id = GUI_GetTextureResource("ClassB.png", tex_Linear | tex_Mipmap, NULL);
        break;
    case width_C:
        id = GUI_GetTextureResource("ClassC.png", tex_Linear | tex_Mipmap, NULL);
        break;
    case width_D:
        id = GUI_GetTextureResource("ClassD.png", tex_Linear | tex_Mipmap, NULL);
        break;
    case width_E:
        id = GUI_GetTextureResource("ClassE.png", tex_Linear | tex_Mipmap, NULL);
        break;
    case width_F:
        id = GUI_GetTextureResource("ClassF.png", tex_Linear | tex_Mipmap, NULL);
        break;
    }
...
bool WED_ATCLayer::DrawEntityStructure(bool inCurrent, IGISEntity* entity, GUI_GraphState* g, bool selected,
                                       bool locked)
{
    ...
    else if (entity->GetGISSubtype() == WED_TaxiRoute::sClass)
    {
        ...
        Point2 label_xy =
            (pts.size() & 1) ? pts[pts.size() / 2] : Midpoint2(pts[pts.size() / 2 - 1], pts[pts.size() / 2]);
        ...
        {
            std::string nam;
            if (seg->GetRunway() != atc_rwy_None) // ideally this would not be needed, but cant figure a way to fix up
                                                  // name upon earth.wed.xml import
            ...
            if (!nam.empty())
            {
                if (mtr1 > 20 && ends[0].squared_distance(ends[1]) >
                                     sqr(20 + 6.0 * nam.size())) // draw labels only if segment long enough
                {
                    glPushMatrix();
